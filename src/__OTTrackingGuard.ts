...
const ENABLED_SERVICES = ENV_TRACKING_SERVICES
  ? ENV_TRACKING_SERVICES.split(',').map((s) => s.trim())
  : []
const TRACKERS_MODULES = {
  matomo,
}

class OTTrackingGuard {
  _consents: TrackingServiceConsent[] = []
  _pendingActions: TrackingActionItem[] = []

  constructor() {
    const browserConsents: TrackingServiceConsent[] =
      ls.getItem(LS_ITEM_NAME) || []
    const consents: TrackingServiceConsent[] = [
      ...browserConsents.filter((c) => ENABLED_SERVICES.includes(c.name)),
    ]
    ENABLED_SERVICES.forEach((srv) => {
      if (browserConsents.every((s) => s.name !== srv))
        consents.push({ name: srv, consent: false, reviewed: false })
    })
  ...
  setConsent(value: boolean, service: string): void {
    const existing: TrackingServiceConsent[] = this._consents.filter(
      (s) => s.name === service,
    )
    if (!existing.length)
      throw new Error(`GDPR: '${service}' service is not enabled.`)
  ...
  hasConsent(service: string): boolean {
    if (this._isServiceEnabled(service))
      return !!this._consents.filter((c) => c.name === service && c.consent)
        .length
    return true
  }
  isReviewed(service = '') {
    if (!service) return this._consents.every((c) => c.reviewed)
    return !!this._consents.filter((c) => c.name === service && c.reviewed)
      .length
  }
